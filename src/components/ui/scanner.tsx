...
export function Scanner({
  ...
  useEffect(() => {
    ...
    // A continuously animating WebGL field is exactly what reduced-motion is
    // for. Skip the whole renderer rather than paint a still frame, so no GPU
    // context is created for a user who asked not to see motion.
    ...
    return () => {
      ...
      try {
        ...
        // Already detached by React; nothing to clean up.
      }
  ...
  useEffect(() => {
    ...
    const apply = (uniform: { value: Float32Array }, hex: string) => {
      const [r, g, b] = hexToRgb(hex);
      uniform.value[0] = r;
      uniform.value[1] = g;
      uniform.value[2] = b;
    };
    apply(u.uColor1 as { value: Float32Array }, color1);
    apply(u.uColor2 as { value: Float32Array }, color2);
    apply(u.uColor3 as { value: Float32Array }, color3);
