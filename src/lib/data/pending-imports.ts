...
/** Falls back to the user's "Other" category so confirming an import is
 * always a single tap. Categorising it precisely can happen later by
 * editing the expense, and blocking the import on a dropdown defeats the
 * point of auto-importing in the first place. */
async function resolveCategoryId(userId: string, requested?: string | null): Promise<string> {
  if (requested) {
